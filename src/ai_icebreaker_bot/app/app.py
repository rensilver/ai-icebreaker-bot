...
# Força o uso de um diretório de cache limpo no seu usuário
os.environ["NLTK_DATA"] = os.path.expanduser("~/nltk_data")

# Baixa os pacotes necessários diretamente para a nova pasta
nltk.download('stopwords', download_dir=os.environ["NLTK_DATA"])
nltk.download('punkt_tab', download_dir=os.environ["NLTK_DATA"])

from ai_icebreaker_bot.loading.data_extraction import extract_linkedin_profile
from ai_icebreaker_bot.processing.data_processing import split_profile_data, create_vector_database, verify_embeddings

from ai_icebreaker_bot.llm.llm_interface import change_llm_model
from ai_icebreaker_bot.retrieval.query_engine import generate_initial_facts, answer_user_query
...
def chat_with_profile(session_id, user_query, chat_history):
    ...
    if not session_id:
        ...
        chat_history.append({"role": "assistant", "content": "No profile loaded. Please process a LinkedIn profile first."})
        return chat_history
    
    if session_id not in active_indices:
        chat_history.append({"role": "user", "content": user_query})
        chat_history.append({"role": "assistant", "content": "Session expired. Please process the LinkedIn profile again."})
        return chat_history
