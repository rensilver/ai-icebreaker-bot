...
from ai_icebreaker_bot.llm.llm_interface import create_nomic_embedding
...
def split_profile_data(profile_data: Dict[str, Any]) -> List:
    ...
    """
    try:
        # Create a JSON string from profile data
        json_string = json.dumps(profile_data)

        # Create a object document from JSON string
...
def create_vector_database(nodes: List) -> Optional[VectorStoreIndex]:
    ...
    try:
        ...
        embedding_model = create_nomic_embedding()
